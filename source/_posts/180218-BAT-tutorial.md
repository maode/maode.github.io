...
小知识：在DOS环境下，可执行文件的优先级由高到低依次为`.com>.exe>.bat>.cmd`，即如果在同一目录下存在文件名相同的这四类文件，当只键入文件名时，DOS执行的是`name.com`，如果需要执行其他三个文件，则必须指定文件的全名，如name.bat。
...
**查阅方法：在 阅读全文 页面 复制目录中的条目，搜索即可。或点右下角的目录链接**

整理者邮箱：[xiangkg@163.com](xiangkg@163.com)，如果你要全文或有好的dos使用经验，请你发email给我，谢谢！

 全文：[http://xiangkg.blog.163.com](http://xiangkg.blog.163.com/)
...
第二章 FOR命令详解
 1. 基本格式
 2. 参数 /d 主要搜索匹配目录
 3. 参数 /R 搜索指定目录树中的匹配文件
 4. 参数 /L 按数字序列(start,step,end)循环
 5. 参数 /F 处理命令输出. 字符串及文件内容
